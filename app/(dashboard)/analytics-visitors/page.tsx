...
export default function VisitorsAnalyticsPage() {
  ...
  const loadChartData = useCallback(async () => {
    try {
      const { data, error: err } = await supabase.rpc('get_visitors_growth')
  ...
  const loadVisitors = useCallback(async (page: number, query: string) => {
    ...
    const { data, error: err } = await supabase.rpc('get_visitors_list', {
      p_search: query || null,
      p_from_row: from,
      p_to_row: to,
    })
                  ...
                  return (
                    <tr key={v.id}>
                      ...
                      <td>
                        {v.whatsapp_number ? (
                          <a
                            href={getWaLink(v.whatsapp_number, '')}
                            target="_blank"
                            rel="noopener noreferrer"
                            title={`Chat WhatsApp ${formatPhone(v.whatsapp_number, '')}`}
                            style={{
                              color: '#25D366',
                              textDecoration: 'none',
                              display: 'inline-flex',
                      ...
                      <td>
                        <div style={{ whiteSpace: 'nowrap' }}>
                          <span style={{ 
                            ...
                            {v.last_login ? timeAgo(v.last_login) : '-'}
                          </span>
                        </div>
                      </td>
