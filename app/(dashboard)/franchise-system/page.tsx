...
export default function FranchiseSystemPage() {
  const [items, setItems] = useState<FranchiseSystem[]>([])
  const [allItems, setAllItems] = useState<FranchiseSystem[]>([])
  ...
  const handleAdd = async () => {
    ...
    setIsAdding(true)
    const { error: err } = await supabase.from('franchise_system').insert({ nama })
    ...
    setAddName('')
    setIsAdding(false)
    setToast({ message: 'System berhasil ditambahkan', type: 'success' })
    loadItems()
  }

  const startEdit = (item: FranchiseSystem) => {
  ...
  const saveEdit = async (id: number) => {
    ...
    setEditingId(null)
    setToast({ message: 'System berhasil diperbarui', type: 'success' })
    loadItems()
  }
  ...
  const handleDelete = async () => {
    ...
    setToast({ message: 'System berhasil dihapus', type: 'success' })
    loadItems()
  }
  ...
  return (
    <>
                ...
                <tr>
                  <td colSpan={3}>
                    <div className="sf-state">
                      <div className="sf-state-icon">&#128203;</div>
                      <h3>Belum ada system</h3>
                      <p>Tambahkan system pertama menggunakan form di bawah tabel.</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        {!loading && !error && items.length > 0 && (
          <div className="sf-summary">
            {searchQuery && items.length !== allItems.length
              ? `Menampilkan ${items.length} dari ${allItems.length} system`
              : `Total ${allItems.length} system`}
          </div>
        )}
      </div>
      ...
      <DeleteModal
        ...
        title="Hapus System?"
        message={`System "${deleteModal.name}" akan dihapus permanen. Pastikan tidak ada merchant yang masih menggunakan system ini.`}
