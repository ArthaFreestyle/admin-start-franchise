...
export default function FranchiseKeunggulanPage() {
  ...
  const loadItems = useCallback(async (page: number, search: string, merchantId: string) => {
    ...
    let q = supabase
      .from('franchise_keunggulan_merchant')
  ...
  const handleAdd = async () => {
    ...
    setAddName('')
    setIsAdding(false)
    setToast({ message: 'Keunggulan berhasil ditambahkan', type: 'success' })
    loadItems(currentPage, searchQuery, filterMerchantId)
  }

  const startEdit = (item: KeunggulanItem) => {
  ...
  const saveEdit = async (id: number) => {
    ...
    setEditingId(null)
    setToast({ message: 'Keunggulan berhasil diperbarui', type: 'success' })
    loadItems(currentPage, searchQuery, filterMerchantId)
  }
  ...
  const handleDelete = async () => {
    ...
    setToast({ message: 'Keunggulan berhasil dihapus', type: 'success' })
    loadItems(currentPage, searchQuery, filterMerchantId)
  }
    ...
    <>
      ...
      <div className="sf-card">
                ...
                <tr>
                  <td colSpan={4}>
                    <div className="sf-state">
                      <div className="sf-state-icon">&#128203;</div>
                      <h3>Belum ada data keunggulan</h3>
                      <p>Tambahkan keunggulan merchant menggunakan form di bawah tabel.</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        <div className="sf-add-panel">
          <div className="sf-add-panel-title">Tambah Keunggulan Baru</div>
          <div className="sf-add-form">
            ...
            <div className="sf-add-field" style={{ flex: 1, minWidth: '200px' }}>
              ...
              <input
                type="text"
                className="sf-inline-input"
                placeholder="Contoh: Bebas royalty fee, BEP Cepat..."
                value={addName}
        ...
        {!loading && !error && items.length > 0 && (
          <div className="sf-summary">
            Menampilkan {(currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, totalCount)} dari {totalCount} keunggulan
          </div>
        )}
      ...
      <DeleteModal
        ...
        message={`Item keunggulan "${deleteModal.name}" ini akan dihapus permanen.`}
