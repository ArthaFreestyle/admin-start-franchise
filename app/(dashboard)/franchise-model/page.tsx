...
export default function FranchiseModelPage() {
  const [items, setItems] = useState<FranchiseModel[]>([])
  const [allItems, setAllItems] = useState<FranchiseModel[]>([])
  ...
  const handleAdd = async () => {
    ...
    setIsAdding(true)
    const { error: err } = await supabase.from('franchise_model').insert({ nama })
    ...
    setAddName('')
    setIsAdding(false)
    setToast({ message: 'Model berhasil ditambahkan', type: 'success' })
    loadItems()
  }

  const startEdit = (item: FranchiseModel) => {
  ...
  const saveEdit = async (id: number) => {
    ...
    setEditingId(null)
    setToast({ message: 'Model berhasil diperbarui', type: 'success' })
    loadItems()
  }
  ...
  const handleDelete = async () => {
    ...
    setToast({ message: 'Model berhasil dihapus', type: 'success' })
    loadItems()
  }
  ...
  return (
    <>
                ...
                <tr>
                  <td colSpan={3}>
                    <div className="sf-state">
                      <div className="sf-state-icon">&#128203;</div>
                      <h3>Belum ada model</h3>
                      <p>Tambahkan model pertama menggunakan form di bawah tabel.</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        {!loading && !error && items.length > 0 && (
          <div className="sf-summary">
            {searchQuery && items.length !== allItems.length
              ? `Menampilkan ${items.length} dari ${allItems.length} model`
              : `Total ${allItems.length} model`}
          </div>
        )}
      </div>
      ...
      <DeleteModal
        ...
        title="Hapus Model?"
        message={`Model "${deleteModal.name}" akan dihapus permanen. Pastikan tidak ada merchant yang masih menggunakan model ini.`}
